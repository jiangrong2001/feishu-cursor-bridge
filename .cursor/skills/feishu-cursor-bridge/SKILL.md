...
可选：`CURSOR_AGENT_WORKSPACE`（绝对路径，默认可为仓库根）、`ALLOWED_SENDER_OPEN_IDS`、`CURSOR_AGENT_STREAM_TO_FEISHU`、`CURSOR_AGENT_TIMEOUT_MS` 等见 [reference.md](reference.md)。
...
失败时按 README **§八、故障排查**；可先看 `[cursor-agent] failed` 与 `agent login` / `lark-cli`。

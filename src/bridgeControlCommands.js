...
function controlCommandsHelpBodyZh() {
  return [
    ...
    "",
    "· /v、/verbos、/verbose",
    "  前缀后须空格或换行再写正文。默认去掉前缀后交给 **Cursor Agent**；飞书**按段推送**对话摘录（与 agent-*.chat.txt 同结构）。",
    "  可与 **/restart** 组合：如 /v /restart 或 /v /restart <目录> —— **仍由桥接执行重启**（不启动 Agent），飞书按段推送**桥接侧**步骤（写 override、fork 进程等）；间隔同 CURSOR_AGENT_TRANSCRIPT_MIN_INTERVAL_MS。",
    "  与环境变量 CURSOR_AGENT_STREAM_TO_FEISHU=1 的进度推送**不叠加**（用 /v 时不再发那套 🔧/📝）。",
    "  可调 CURSOR_AGENT_TRANSCRIPT_MIN_INTERVAL_MS（默认 350ms）控制摘录消息间隔。",
    "",
    "· /restart",
    "  可加 /v、/verbos、/verbose 前缀：例如 /v /restart <目录> —— **重启仍由桥接完成**；带 /v 时飞书会多段显示桥接执行过程（非 Agent）。",
